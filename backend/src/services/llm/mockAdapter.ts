...
export class MockLlmAdapter implements LlmAdapter {
  ...
  private async fetchWithRetry(
    ...
    try {
      logger.info(`Calling mock LLM (attempt ${attempt + 1}/${this.maxRetries + 1})`);
      ...
      if ((isTimeout || is500Error) && attempt < this.maxRetries) {
        ...
        logger.warn(
          `LLM call failed (${isTimeout ? 'timeout' : '500 error'}), retrying in ${delay}ms...`
        );
      ...
      logger.error('LLM call failed after all retries', { error: error.message });
      throw error;
    }
  }
}
